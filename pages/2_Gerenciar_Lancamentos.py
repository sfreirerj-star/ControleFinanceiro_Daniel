...
st.set_page_config(
    page_title="Gerenciar Lançamentos — Painel do Daniel", page_icon="✏️"
)
...
def configurar_sidebar_competencia():
  try:
    ...
    df_d = pd.read_sql_query("SELECT id FROM dividas", conexao)
    conexao.close()
  except Exception:
    df_l = pd.DataFrame(columns=["data"])
    df_d = pd.DataFrame(columns=["id"])
...
# Identificação correta do Painel do Daniel
...
# Função para extrair competência
def extrair_competencia(data_str):
  try:
    dt = pd.to_datetime(data_str, format="%d/%m/%Y", errors="coerce")
    if pd.isna(dt):
...
if not df.empty:
  ...
  # Filtrar lançamentos da competência selecionada para gerenciar/editar
  ...
  # SEÇÃO INTELIGENTE: Puxa as parcelas ativas das dívidas
  if not df_div.empty:
    df_div_ativas = df_div[df_div["valor_parcela"] > 0].copy()

    if not df_div_ativas.empty:
      # Baseado na competência selecionada (MM/YYYY)
      try:
...
if not df.empty:
    ...
    st.info("Nenhum lançamento encontrado para gerenciar.")
>>>>>>> a968adb (Adicionadas as paginas de dividendos e mercado internacional)
