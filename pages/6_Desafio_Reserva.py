...
st.write(
    "Controle seus aportes mensais cumulativos, diversifique suas instituições"
    " financeiras, analise a evolução histórica por mês e simule o impacto de"
    " ganhos extras nas suas metas."
)
...
with aba_desafio:
    with st.expander(
        ...
        opcoes_meta_mensal = {
            ...
            "CPROEIS Soldado/Cabo - Serviço de 6h (R$ 269,89)": 269.89,
            "CPROEIS Soldado/Cabo - Serviço de 8h (R$ 333,73)": 333.73,
            "CPROEIS Soldado/Cabo - Serviço de 12h (R$ 461,41)": 461.41,
            "CPROEIS Soldado/Cabo - Serviço de 24h (R$ 844,47)": 844.47,
    ...
    with st.form("form_novo_aporte"):
        ...
        with col_a2:
            valor_aporte = st.number_input(
                "Valor Depositado (R$)",
                min_value=1.0,
                value=meta_mensal_base if meta_mensal_base > 0 else 100.0,
                step=10.0,
                format="%.2f",
            )
        with col_a3:
    ...
    if not df_aportes.empty:
        ...
        res_aportes = df_aportes["data"].apply(extrair_mes_ano)
        df_aportes["mes_ano"] = [x[0] for x in res_aportes]
        df_aportes["ordem_mes"] = [x[1] for x in res_aportes]

        df_grafico = (
            df_aportes[df_aportes["mes_ano"] != "Indefinido"]
            .groupby(["mes_ano", "ordem_mes"])["valor_num"]
            .sum()
            .reset_index()
            .sort_values("ordem_mes", ascending=True)
        )

        if not df_grafico.empty:
            st.write(
                "Veja abaixo o total de aportes realizados em cada mês/ano:"
            )
            st.bar_chart(
                df_grafico.set_index("mes_ano")[["valor_num"]].rename(
                    columns={"valor_num": "Total Aportado no Mês (R$)"}
                )
            )
        else:
            st.info("Nenhum dado com data válida para gerar o gráfico mensal.")
    else:
        st.info("Cadastre aportes para visualizar o gráfico comparativo mensal.")
...
with aba_simulador:
    ...
    with col_s1:
        ...
        aporte_mensal_sim = st.number_input(
            ...
            min_value=100.0,
            value=meta_mensal_base if meta_mensal_base > 0 else 461.41,
            step=100.0,
            format="%.2f",
        )
    with col_s2:
